...
public class DishServiceImpl implements DishService {
    ...
    public void deleteByIds(List<Long> ids) {
        for (Long id : ids) {
            ...
            if (dish == null) {
                throw new BusinessException("要删除的菜品不存在！请刷新后重试");
            }
            ...
            //3. 被套餐关联的菜品不能删除
            Long counts = setmealDishMapper.selectCount(new LambdaQueryWrapper<SetmealDish>().eq(SetmealDish::getDishId, dish.getId()));
            if (counts != 0L) {
                throw new BusinessException("该菜品存在于套餐中，请先从套餐中移除该菜品");
            }
            ...
            // 4.1 从缓存中删除
            if (Boolean.TRUE.equals(redisTemplate.delete("dish:" + dish.getCategoryId()))) {
                // 可能该菜品没有被缓存
                log.info("{}删除菜品缓存失败", getClass());
            }else{
                log.info("菜品缓存删除失败，可能该菜品还没有被缓存");
            }
